...
\end{equation}
where $C_k(X) = \begin{pmatrix} a_p & 1 \\ -\Phi_{p^k}(1+X) & 0 \end{pmatrix}$, $A = \begin{pmatrix} a_p & 1 \\ -p & 0 \end{pmatrix}$ is the Frobenius action matrix at $X=0$, and $\alpha, \bar\alpha$ are the roots of $X^2 - a_p X + p$. Equations~\eqref{eq:halflog-matrix} and~\eqref{eq:H-def} follow Sprung's Definitions~6.8 and~4.5 directly; no modification of his framework is required.
...
On the Dieudonn\'e module $D_p(E) = H^1_{\rm dR}(E/\Qp)$, the Frobenius operator $\varphi$ admits the matrix representation
\begin{equation}\label{eq:phi-matrix}
\varphi = \begin{pmatrix} 0 & -1/p \\ 1 & a_p/p \end{pmatrix}
\end{equation}
in the \emph{cyclic} basis $(\omega, \varphi\omega)$, where $\omega$ is the invariant differential. This is the companion matrix of the characteristic polynomial $X^2 - (a_p/p)X + 1/p = 0$ of $\varphi$, and it satisfies $\varphi^2 - (a_p/p) \varphi + 1/p = 0$.

\begin{remark}[Two distinct Frobenius representations in SageMath]\label{rem:two-bases}
SageMath uses two different bases for the Dieudonn\'e module, leading to two different matrix representations of the same operator $\varphi$:
\begin{enumerate}
\item[(a)] The \emph{geometric} basis $(\omega, \eta)$ with $\eta = x\,\omega$, returned by the method \texttt{L.frobenius()}.
\item[(b)] The \emph{cyclic} basis $(\omega, \varphi\omega)$, used internally in \texttt{L.Dp\_valued\_series()} via the explicit assignment \texttt{phi = matrix([[0, -1/p], [1, E.ap(p)/p]])} at \texttt{padic\_lseries.py}, line 1442.
\end{enumerate}
The matrix~\eqref{eq:phi-matrix} represents $\varphi$ in basis (b); this is the convention we use throughout, since the bridge formula of Theorem~\ref{thm:bridge} concerns exactly the output of \texttt{Dp\_valued\_series}. A reader using \texttt{L.frobenius()} to obtain~\eqref{eq:phi-matrix} would obtain a different matrix, related to ours by an explicit change-of-basis.
\end{remark}

We will encounter the matrix $\eps^{-1} := (1 - \varphi)^2$ throughout. Direct computation gives:
...
The convergence of the projective limit~\eqref{eq:H-def} for general $X$ is the content of Sprung's \cite[Lemma~4.4 (Convergence Lemma) and Lemma~4.8 (Growth Lemma)]{Sprung2012}, and is reformulated by Lei~\cite[Theorem~1.5]{Lei2013} in the language of $p$-adic logarithmic matrices. Theorem~\ref{thm:trivial-limit} above is the specialisation $X = 0$, where the limit becomes algebraically trivial. The determinant of $\Hcal(X)$ is computed in~\cite[Example~4.7]{Sprung2012}; for our application at $X = 0$ it reduces to $\det(A^{-2}) = 1/p^2$, in agreement with Lemma~\ref{lem:det-eps} via the bridge $\eps = (1-\varphi)^2$.
\end{remark}
...
\begin{proof}
The first equation is the docstring statement of \texttt{Dp\_valued\_series} (\texttt{padic\_lseries.py}, class \texttt{pAdicLseriesSupersingular}, lines 1398--1447).

For the second equation, we first establish the algebraic identification between $L_p(E, \alpha, T) = G(T) + H(T)\alpha \in \Qp(\alpha)[\![T]\!]$ and an element of $D_p(E)[\![T]\!]$ in the cyclic basis $(\omega, \varphi\omega)$ of Section~\ref{ssec:dieudonne}. There is a unique embedding $\Qp(\alpha) \hookrightarrow D_p(E)[1/p] \otimes \Qp(\alpha)$ sending $1 \mapsto \omega$ and respecting the multiplication $\alpha^2 = a_p\alpha - p$ (the minimal polynomial of $\alpha$ on the Tate module). Concretely, this embedding sends
\begin{equation}\label{eq:embedding-alpha}
\alpha \longmapsto a_p\,\omega - p\,\varphi(\omega).
\end{equation}
To verify~\eqref{eq:embedding-alpha}, compute the image of $\alpha^2$ on both sides: on the source side, $\alpha^2 = a_p\alpha - p$ maps to $a_p\bigl(a_p\omega - p\varphi(\omega)\bigr) - p\,\omega = (a_p^2 - p)\,\omega - a_p\,p\,\varphi(\omega)$; on the target side, $\bigl(a_p\omega - p\varphi(\omega)\bigr)\cdot\bigl(a_p\omega - p\varphi(\omega)\bigr)$ in the ring $\Qp[\varphi]/(\varphi^2 - (a_p/p)\varphi + 1/p)$ equals $a_p^2\,\omega - 2 a_p p\,\varphi(\omega) + p^2\varphi^2(\omega) = a_p^2\,\omega - 2 a_p p\,\varphi(\omega) + p^2\bigl((a_p/p)\varphi(\omega) - (1/p)\omega\bigr) = (a_p^2 - p)\,\omega - a_p p\,\varphi(\omega)$, matching the source.

Under~\eqref{eq:embedding-alpha}, the element $L_p = G + H\alpha$ maps to
\[
G\cdot\omega + H\cdot(a_p\omega - p\varphi(\omega)) = (G + a_p H)\,\omega + (-pH)\,\varphi(\omega).
\]
Multiplying~\eqref{eq:sage-def} from the right by $(1-\varphi)^2$ removes the normalisation factor, yielding $L_p = (G + a_p H)\,\omega + (-pH)\,\varphi(\omega)$ in the cyclic basis. Solving for $G$ and $H$ from the components of $\text{sp} \cdot ((1-\varphi)^2)^T$ gives~\eqref{eq:bridge-explicit}. This is exactly the inverse of the assignment in \texttt{padic\_lseries.py}, lines~1442--1445.
\end{proof}
...
\hline
\end{tabular}
\caption{$(G_{V_0}, H_{V_0})$ for seven supersingular curves at $p = 2$, at the first stage $V_0$ where $L_p^\flat$ does not vanish (cf.\ Remark~\ref{rem:V0-def}). Values are recovered from SageMath's \texttt{Dp\_valued\_series} via the bridge formula~\eqref{eq:bridge-explicit}; all seven pairs lie in $\Q^2$ (a consequence of Theorem~\ref{thm:bridge} and the rationality of SageMath's modular-symbol output). The sample was chosen to span the full conductor range of the verified set; the remaining $173$ curves yield analogous rational $(G_{V_0}, H_{V_0})$-pairs and are tabulated in the supplementary code repository. A reproducer script is provided at \texttt{code/sage\_verify\_paper\_table1.py}.}
\label{tab:sample}
\end{table}

\begin{remark}[Definition of $V_0$ and rationality]\label{rem:V0-def}
We use $V_0 = V_0^\flat$ to denote the first stage $k \geq 0$ at which $L_p^\flat$ is nonzero; equivalently, the smallest index $k$ for which the second component $v_1[k]$ of the SageMath output is nonzero. The Mazur--Tate stage $V_0$ thus depends on the curve and is in general distinct from $X = 0$ (which is treated in Theorem~\ref{thm:trivial-limit}); the table entries below are values of $L_p$ at $T^{V_0}$, not at $T = 0$. The rational form of each entry follows from Theorem~\ref{thm:bridge}, since SageMath's modular-symbol output is rational and the bridge formula~\eqref{eq:bridge-explicit} preserves rationality.
\end{remark}

\begin{remark}[Asymmetric vanishing of $L_p^\sharp$ and $L_p^\flat$]\label{rem:V0-asymmetry}
In general, $L_p^\sharp$ may vanish to a different order than $L_p^\flat$; let $V_0^\sharp$ and $V_0^\flat = V_0$ denote the respective vanishing levels. The curve \texttt{55935g1} provides a concrete instance with $V_0^\sharp = 4$ while $V_0^\flat = 5$: SageMath's $v_0[4] = 1/2 \neq 0$ at one stage before $v_1$ becomes nonzero. The asymmetric vanishing phenomenon is intrinsic to Sprung's framework and is discussed in~\cite[Section~6]{Sprung2012}. For all seven curves in Table~\ref{tab:sample}, the $(G_{V_0}, H_{V_0})$-pair is recovered at the $L_p^\flat$-vanishing stage $V_0^\flat$.
\end{remark}
...
\end{tabular}
\caption{Numerical verification of Theorem~\ref{thm:bridge}: for each curve, the inverse bridge formula~\eqref{eq:bridge-explicit} yields a rational $(G_k, H_k)$ at every Mazur--Tate stage $k$, in bit-exact agreement with SageMath's \texttt{Dp\_valued\_series} output. Verified on 180 curves spanning four orders of magnitude in conductor; zero discrepancies.}
\label{tab:bilanz}
\end{table}
...
\subsection{Relation to Sprung's distribution-matrix reformulation}\label{subsec:sprung2022}
After the present work was completed, the author became aware of Sprung's recent reformulation~\cite{Sprung2022} of the logarithm matrix in terms of distributions on $\Z_p$, characterised combinatorially by $p$-adic digits and extending the Dion--Lei framework~\cite{DionLei2017} from $a_p = 0$ to general $a_p$. The two perspectives are complementary: \cite{Sprung2022} treats $\mathrm{Log}_{a_p}(1+T)$ as a distribution-valued object across all of $\Z_p$, whereas the present work focuses on the explicit closed-form evaluation $\Hcal(0) = A^{-2}$ at $T = 0$ and its computational interface with SageMath at $p = 2$. The normalisation conventions differ: \cite{Sprung2022} uses an auxiliary root matrix $R_n$ with a different power-of-$p$ scaling for $p = 2$ (see~\cite[footnote~1]{Sprung2022}); at $X = 0$ the two normalisations are related by a unimodular transformation depending on $a_p$, leaving characteristic ideals invariant. A direct comparison of the explicit values of Corollary~\ref{cor:rationality} in the digit-distribution language of~\cite{Sprung2022} would be of interest.
...
Canad.\ Math.\ Bull.\ \textbf{57} (2014), no.~4, 845--852.
arXiv: \href{https://arxiv.org/abs/1304.7473}{1304.7473}.

\bibitem{DionLei2017}
C.~Dion and A.~Lei,
\emph{Plus and minus logarithms and Amice transform,}
C.~R.\ Math.\ Acad.\ Sci.\ Paris \textbf{355} (2017), no.~9, 942--948.

\bibitem{Sprung2022}
F.~E.~I.~Sprung,
\emph{La matrice de logarithme en termes de chiffres $p$-adiques (The logarithm matrix in terms of $p$-adic digits),}
Ann.\ Math.\ Qu\'ebec, published online 2023.
DOI: \href{https://doi.org/10.1007/s40316-023-00215-2}{10.1007/s40316-023-00215-2}.
arXiv: \href{https://arxiv.org/abs/2211.08334}{2211.08334}.
