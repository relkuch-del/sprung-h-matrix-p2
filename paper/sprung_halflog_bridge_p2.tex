...
\end{document}
dge}: for each curve, the inverse bridge formula~\eqref{eq:bridge-explicit} applied to SageMath's \texttt{Dp\_valued\_series} output yields a rational $(G_k, H_k)$ at every Mazur--Tate stage $k$, and the corresponding identity in the cyclic basis holds bit-exactly. Verified on 180 curves spanning four orders of magnitude in conductor; zero discrepancies.}
\label{tab:bilanz}
\end{table}
