...
\end{equation}
where $C_k(X) = \begin{pmatrix} a_p & 1 \\ -\Phi_{p^k}(1+X) & 0 \end{pmatrix}$, $A = \begin{pmatrix} a_p & 1 \\ -p & 0 \end{pmatrix}$ is the Frobenius action matrix at $X=0$, and $\alpha, \bar\alpha$ are the roots of $X^2 - a_p X + p$. (Our normalisation of $\Hcal(X)$ differs from~\cite[Definition~4.5]{Sprung2012} by absorption of the matrix $B = \begin{pmatrix} -1 & -1 \\ \beta & \alpha \end{pmatrix}$ into the right factor of~\eqref{eq:halflog-matrix}; the resulting half-logarithms coincide.)
...
\end{tabular}
\caption{Numerical verification of Theorem~\ref{thm:bridge}: for each curve, the inverse bridge formula~\eqref{eq:bridge-explicit} applied to SageMath's \texttt{Dp\_valued\_series} output yields a rational $(G_k, H_k)$ at every Mazur--Tate stage $k$, and the relation $\text{sp} = ((G + a_p H,\,-pH)) \cdot ((1-\varphi)^2)^T / (1-\varphi)^2 = L_p / (1-\varphi)^{-2}$ holds bit-exactly. Verified on 180 curves spanning four orders of magnitude in conductor; zero discrepancies.}
\label{tab:bilanz}
\end{table}
...
\end{document}
dge}: for each curve, the inverse bridge formula~\eqref{eq:bridge-explicit} applied to SageMath's \texttt{Dp\_valued\_series} output yields a rational $(G_k, H_k)$ at every Mazur--Tate stage $k$, and the corresponding identity in the cyclic basis holds bit-exactly. Verified o